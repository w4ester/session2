...
## 🔐 OAuth Setup for Custom GPT (Simplified Method)

**This method uses ChatGPT's built-in OAuth flow instead of manually generating access tokens.**

### Quick Links You'll Need

1. **LinkedIn Developer Portal:** https://www.linkedin.com/developers/apps
2. **Custom GPT Editor:** https://chatgpt.com/gpts/editor
3. **LinkedIn OAuth Documentation:** https://learn.microsoft.com/en-us/linkedin/shared/authentication/authentication
...
3. Add this redirect URL:
   ...
   ```
   **Note:** You'll get your actual GPT ID from the Custom GPT editor URL. It looks like: `https://chatgpt.com/gpts/editor/g-abc123xyz`

4. **OR** use ChatGPT's generic OAuth callback (if your GPT ID isn't working):
   ```
...
1. Still in the **"Auth"** tab of your LinkedIn app
2. Copy these two values (you'll need them in a moment):
   - **Client ID** (long string like: `77abcxyz123456`)
   - **Client Secret** (click "Show" to reveal it)

#### Part 4: Request LinkedIn Permissions
...
2. Find and request access to:
   ...
   - ✅ **"Share on LinkedIn"** (for posting ability)
3. Click **"Request access"** for each
4. Approval is usually instant for personal use

#### Part 5: Configure OAuth in Custom GPT

1. Go to https://chatgpt.com/gpts/editor
2. Open your GPT (or create a new one)
3. Go to **"Configure"** tab
4. Scroll down to **"Actions"** section
5. Click **"Add actions"** (or edit existing action)
6. Paste in your OpenAPI schema for LinkedIn posting (from the workshop template)
7. Scroll down to **"Authentication"**
8. Click **"Authentication"** dropdown
9. Select **"OAuth"**

#### Part 6: Fill in OAuth Configuration

Now enter these values exactly:

**Authentication Type:** OAuth

**Client ID:**
```
[Paste your Client ID from LinkedIn app]
```

**Client Secret:**
```
[Paste your Client Secret from LinkedIn app]
```
...
1. Click **"Save"** in the Custom GPT editor
2. When you first try to use the LinkedIn posting action, ChatGPT will prompt you to authenticate
3. Click **"Authenticate"** or **"Sign in to LinkedIn"**
4. You'll be redirected to LinkedIn to authorize the app
5. After authorizing, you'll be redirected back to ChatGPT
6. The OAuth token will be stored securely

#### Part 8: Verify It Works

1. In your GPT chat, ask: "Draft a test LinkedIn post"
2. Review the generated post
3. Say: "Post this to LinkedIn"
4. If configured correctly, you'll be prompted to authenticate on first use
5. After authentication, the post should be sent to LinkedIn

---

### 🔑 OAuth Values Reference
...
| **Redirect URL (in LinkedIn)** | `https://chat.openai.com/aip/g-[YOUR-GPT-ID]/oauth/callback` |
...
- **Solution:** Make sure you added the exact redirect URL to your LinkedIn app's "Redirect URLs" list
- Double-check there are no extra spaces or characters

**Issue: "Invalid Client ID or Secret"**
- **Solution:** Copy the values again from LinkedIn developer portal
- Make sure you're using Client Secret, not other keys

**Issue: "Insufficient Permissions"**
- **Solution:** Go back to LinkedIn app → Products tab → Request "Share on LinkedIn" access

**Issue: "OAuth callback failed"**
- **Solution:** Try using the generic callback URL: `https://chat.openai.com/aip/oauth/callback`

---

## 📝 Alternative Method: Manual Access Token (Legacy)

**If OAuth isn't working, you can use a manual access token instead. This is the "old way" but still works:**
